...
async function handleRequest(req, res){
  ...
  if(pathname==='/api/damage-reports' && req.method==='POST'){
    if(!['Admin','CEO','FinanceManager'].includes(actor.role)) return deny(res,403,`Role "${actor.role}" cannot create damage reports — manager tier required (same gate as a manual Inventory Adjustment).`,{userId:actor.id,role:actor.role,path:pathname});
    const r = D.createDamageReport({...body, actor}); return sendJson(res, r.ok?200:400, r);
  }
  ...
  if(pathname==='/api/purchase-vendor-report' && req.method==='GET'){
    ...
    return sendJson(res,200,{ok:true, rows:D.purchaseVendorReport(parsed.query.vendorId||null)});
  }
  ...
  if(pathname==='/api/labour-wages' && req.method==='POST'){
    if(!(actor.role==='ProjectManager' && isProjectManagerOf(actor,body.projectId)) && !['Admin','CEO','FinanceManager','Accountant'].includes(actor.role)){
      return deny(res,403,`Role "${actor.role}" cannot record labour wages for project ${body.projectId}.`,{userId:actor.id,role:actor.role,path:pathname});
    }
    const r = D.recordLabourWages({...body, actor}); return sendJson(res, r.ok?200:400, r);
  }
  ...
  if(pathname==='/api/project-expenses' && req.method==='POST'){
    if(!(actor.role==='ProjectManager' && isProjectManagerOf(actor,body.projectId)) && !['Admin','CEO','FinanceManager','Accountant'].includes(actor.role)){
      return deny(res,403,`Role "${actor.role}" cannot record an expense for project ${body.projectId}.`,{userId:actor.id,role:actor.role,path:pathname});
    }
    const r = D.recordProjectExpense({...body, actor}); return sendJson(res, r.ok?200:400, r);
  }
  ...
  if(pathname==='/api/purchase-requisitions' && req.method==='POST'){
    if(!SOP_SITE_ROLES.has(actor.role)) return deny(res,403,`Role "${actor.role}" cannot create a Purchase Requisition.`,{userId:actor.id,role:actor.role,path:pathname});
    const r = D.createPurchaseRequisition({...body, actor}); return sendJson(res, r.ok?200:400, r);
  }
  ...
  if(pathname==='/api/payment-approval-matrix' && req.method==='POST'){
    if(!SOP_FINANCE_ROLES.has(actor.role)) return deny(res,403,`Role "${actor.role}" cannot configure the Payment Approval Matrix.`,{userId:actor.id,role:actor.role,path:pathname});
    if(body.tiers) D.DB.paymentApprovalMatrix.tiers = body.tiers;
    D.DB.paymentApprovalMatrix.finalised = false; // SOP itself: "To Be Finalised" — never settable to true from this route
    D.save();
    D.logAudit({type:'PaymentApprovalMatrixChanged', matrix:D.DB.paymentApprovalMatrix, userId:actor.id, role:actor.role});
    return sendJson(res,200,{ok:true, matrix:D.DB.paymentApprovalMatrix});
  }
  ...
  if(pathname==='/api/site-material-consumption' && req.method==='POST'){
    if(!SOP_SITE_ROLES.has(actor.role)) return deny(res,403,`Role "${actor.role}" cannot record Site Material Consumption.`,{userId:actor.id,role:actor.role,path:pathname});
    const r = D.createMaterialIssue({...body, actor}); return sendJson(res, r.ok?200:400, r);
  }
  ...
  if(pathname==='/api/payment-requests' && req.method==='POST'){
    if(!can(actor,'create')) return deny(res,403,`Role "${actor.role}" cannot raise a payment request.`,{userId:actor.id,role:actor.role,path:pathname});
    const r = D.createPaymentRequest({...body, actor}); return sendJson(res, r.ok?200:400, r);
  }
  ...
  if(pathname.match(/^\/api\/petty-cash-floats\/[^/]+\/replenish$/) && req.method==='POST'){
    if(!SOP_FINANCE_ROLES.has(actor.role)) return deny(res,403,`Role "${actor.role}" cannot replenish a Petty Cash Float.`,{userId:actor.id,role:actor.role,path:pathname});
    const r = D.replenishPettyCashFloat({pettyCashFloatId:pathname.split('/')[3], ...body, actor}); return sendJson(res, r.ok?200:400, r);
  }
