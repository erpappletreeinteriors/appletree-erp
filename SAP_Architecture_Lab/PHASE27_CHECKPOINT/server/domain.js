...
const ROLES = ['Admin','CEO','Accountant','FinanceManager','ProjectManager','Purchase','Sales','Estimator','Viewer'];
const ROLE_ACTIONS = {
...
function freshDB(){
  ...
  return {
    ...
    customers: JSON.parse(JSON.stringify(SEED.customers)).map(c=>({...c, active:true, salesOwnerId:null, createdFromLeadId:null, createdFromQuotationId:null, createdAt:null, createdBy:null, gstin:null})),
...
function postSupplierPayment({vendorId, invoiceEntryId, amount, date, narration, actor, overrideReason, paymentMethodId, bankAccountId}){
  ...
  const lines = [ {account:AP_ACCOUNT, debit:+amount, credit:0, vendorId, projectId:apLine.projectId},
    {account:bankGlAccount, debit:0, credit:+amount, vendorId, projectId:apLine.projectId} ];
...
function createPurchaseOrder({projectId, materialRequestId, rfqId, supplierComparisonId, vendorId, lines, deliveryLocation, expectedDate, paymentTerms, terms, actor}){
  ...
  const po = { id:'PO-'+String(DB.purchaseOrders.length+1).padStart(4,'0'), poNo:null, projectId, materialRequestId:materialRequestId||null,
    rfqId:rfqId||null, supplierComparisonId:supplierComparisonId||null, vendorId, lines, total,
  ...
  logAudit({type:'PurchaseOrderCreated', poId:po.id, projectId, vendorId, total, userId:actor.id, role:actor.role});
  return {ok:true, po};
}
...
function postInventoryMovement({type, materialId, qty, uom, warehouseId, projectId, sourceType, sourceId, valuationRate, actor}){
  const mv = { id:'MV-'+String(DB.inventoryMovements.length+1).padStart(6,'0'), type, materialId, qty:+qty, uom:uom||'',
    warehouseId, projectId:projectId||null, sourceType, sourceId, date:new Date().toISOString().slice(0,10),
...
function getStockLevel(materialId, warehouseId){
  return DB.inventoryMovements.filter(m=>m.materialId===materialId && (!warehouseId || m.warehouseId===warehouseId))
    .reduce((s,m)=>{
  ...
  grn.lines.forEach((l,idx)=>{
    ...
    if(+l.qtyAccepted>0){
      ...
      postInventoryMovement({type:'Receipt', materialId:l.materialId, qty:baseQty, uom:baseUom, warehouseId, projectId:po.projectId,
        sourceType:'GRN', sourceId:grn.id, valuationRate:baseRate, actor});
...
function checkThreeWayMatch({poId, grnId, invoiceLines}){
  ...
  invoiceLines.forEach((il,idx)=>{
    ...
    if(Math.abs((+il.qty) - (+grnLine.qtyAccepted)) > 0.001) mismatches.push(`Line ${idx}: qty mismatch — invoice ${il.qty} vs GRN accepted ${grnLine.qtyAccepted}.`);
...
function draftSupplierInvoiceFromPO({poId, grnId, invoiceLines, taxCode, date, narration, authorizedException, exceptionReason, createdByUserId, createdByRole}){
  ...
  if(draft.ok){ draft.draft.poId=poId; draft.draft.grnId=grnId; save(); }
...
function createMaterialIssue({projectId, materialId, qty, warehouseId, purpose, requestedBy, actor, sourceType, sourceId, overrideReason}){
  ...
  if(+qty > available + 0.001) return {ok:false, error:`Cannot issue ${qty} ${material.uom} — only ${available} available in ${warehouseId} (negative stock is blocked, not silently allowed).`};
  const rate = getMovingAverageRate(materialId, warehouseId);
  const mv = postInventoryMovement({type:'Issue', materialId, qty, uom:material.uom, warehouseId, projectId, sourceType:sourceType||'MaterialIssue', sourceId:sourceId||null, valuationRate:rate, actor});
  ...
  logAudit({type:'MaterialIssue', movementId:mv.id, projectId, materialId, qty, value, userId:actor.id, role:actor.role});
  return {ok:true, movement:mv, valuationRate:rate, value, glEntry: glResult.entry||null};
}
...
function projectFinancial360(projectId){
  ...
  return { ok:true, projectId,
    ...
    cost: { standardCost: baseline ? baseline.totalStandardCost : null, committed: costBreakdown.committed, received: costBreakdown.received, actual: r2(materialCost+labourCostAll), forecast: r2(costBreakdown.committed + materialCost + labourCostAll) },
...
function assertCustomerSelectable(customerId){
  const c = DB.customers.find(x=>x.id===customerId);
  if(c && c.active===false) return `Customer "${c.name}" (${customerId}) is INACTIVE and cannot be used on a new transaction.`;
  return null;
}
function assertVendorSelectable(vendorId){
  const v = DB.vendors.find(x=>x.id===vendorId);
  if(v && v.active===false) return `Vendor "${v.name}" (${vendorId}) is INACTIVE and cannot be used on a new transaction.`;
  return null;
}
...
function assertMaterialSelectable(materialId){
  const m = DB.materials.find(x=>x.id===materialId);
  if(m && m.active===false) return `Material "${m.code}" (${materialId}) is INACTIVE and cannot be used on a new transaction.`;
  return null;
}
...
module.exports = {
  ...
  createPurchaseReturn, createSupplierCreditNote, createMaterialIssue, projectCostBreakdown,
