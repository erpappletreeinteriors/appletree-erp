...
async function main(){
  ...
  // Phase 24 Part A: bank import now requires a distinct glAccount for real multi-account GL
  // segregation — a fresh dedicated GL account is created here specifically for this test bank.
  await api('admin','POST','/api/masters/account',{accountCode:'9050', accountName:'Test Bank Account (Phase 20 handover test)', accountType:'Asset'});
  const bankImp = await api('admin','POST','/api/master-import',{importType:'Banks', csvText:'bankName,accountName,glAccount\nTest Bank,Test Account,9050'});
