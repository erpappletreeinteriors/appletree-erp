...
function createDamageReport({materialId, qty, warehouseId, reasonCategory, explanation, actor}){
  ...
  save();
  logAudit({type:'DamageReportCreated', damageReportId:dmg.id, materialId, qty:dmg.qty, warehouseId, reasonCategory, value:dmg.value, itcReversed:dmg.itcReversed, userId:actor.id, role:actor.role});
  return {ok:true, damageReport:dmg, adjustment:adj.adjustment, glEntry:adj.glEntry, itcReversal: itc.ok?{amount:itc.reversed, glEntry:itc.glEntry||null}:null};
}
...
function dispatchToJobWorker({projectId, customerId, jobWorkerId, warehouseId, lines, expectedReturnDate, isCapitalGoods, purpose, jobWorkReference, transporterName, vehicleNo, actor}){
  const jw = DB.jobWorkers.find(x=>x.id===jobWorkerId); if(!jw) return {ok:false, error:'Job Worker not found.'};
  ...
  lines.forEach(l=>{
    ...
    postInventoryMovement({type:'Issue', materialId:l.materialId, qty:l.qty, uom:material.uom, warehouseId, projectId:projectId||null, sourceType:'JobWorkOrder', sourceId:null, valuationRate:rate, actor});
    const mv = postInventoryMovement({type:'JobWorkReceipt', materialId:l.materialId, qty:l.qty, uom:material.uom, warehouseId:null, jobWorkerId, projectId:projectId||null, sourceType:'JobWorkOrder', sourceId:null, valuationRate:rate, actor});
    movementIds.push(mv.id); totalValue += r2(l.qty*rate);
  });
  const dc = { id:'DC-'+String(DB.deliveryChallans.length+1).padStart(4,'0'), dcNo:nextDocNumber('DC'), jobWorkerId, warehouseId,
  ...
  DB.deliveryChallans.push(dc);
  const jwo = { id:'JWO-'+String(DB.jobWorkOrders.length+1).padStart(4,'0'), jwoNo:nextDocNumber('JWO'), projectId:projectId||null, customerId:customerId||null,
...
function returnFromJobWorker({jwoId, returnedLines, actor}){
  ...
  returnedLines.forEach((rl,idx)=>{
    ...
    postInventoryMovement({type:'JobWorkReturn', materialId:line.materialId, qty:+rl.qty, uom:material.uom, warehouseId:null, jobWorkerId:jwo.jobWorkerId, projectId:jwo.projectId, sourceType:'JobWorkOrder', sourceId:jwo.id, valuationRate:rate, actor});
    const mv = postInventoryMovement({type:'Receipt', materialId:line.materialId, qty:+rl.qty, uom:material.uom, warehouseId:jwo.warehouseId, projectId:jwo.projectId, sourceType:'JobWorkOrder', sourceId:jwo.id, valuationRate:rate, actor});
    movementIds.push(mv.id);
...
function recordJobWorkScrap({jwoId, lineIndex, qty, disposition, taxHandlingRef, actor}){
  const jwo = DB.jobWorkOrders.find(x=>x.id===jwoId); if(!jwo) return {ok:false, error:'Job Work Order not found.'};
  ...
  const mv = postInventoryMovement({type:'JobWorkScrap', materialId:line.materialId, qty:+qty, uom:material.uom, warehouseId:null, jobWorkerId:jwo.jobWorkerId, projectId:jwo.projectId, sourceType:'JobWorkOrder', sourceId:jwo.id, valuationRate:rate, actor});
  ...
  const rec = { id:'JWSCRAP-'+String(DB.jobWorkScrapRecords.length+1).padStart(4,'0'), jwoId, lineIndex, materialId:line.materialId, qty:+qty, value:r2(qty*rate),
    jobWorkerId:jwo.jobWorkerId, jobWorkerRegistered:jobWorker.registered, disposition, taxHandlingRef:taxHandlingRef||'', taxFlag, movementId:mv.id, createdBy:actor.id, createdAt:nowIso() };
  DB.jobWorkScrapRecords.push(rec); save();
  logAudit({type:'JobWorkScrapRecorded', jwoId, disposition, qty:+qty, taxFlag, userId:actor.id, role:actor.role});
...
function directDispatchFromJobWorker({jwoId, lineIndex, qty, customerId, actor}){
  const jwo = DB.jobWorkOrders.find(x=>x.id===jwoId); if(!jwo) return {ok:false, error:'Job Work Order not found.'};
  const jobWorker = DB.jobWorkers.find(x=>x.id===jwo.jobWorkerId);
  const line = jwo.lines[lineIndex]; if(!line) return {ok:false, error:'Invalid line index.'};
  const already = (jwo.returnedQtyByLine[lineIndex]||0) + (jwo.scrapQtyByLine[lineIndex]||0);
  ...
  const mv = postInventoryMovement({type:'JobWorkDirectDispatch', materialId:line.materialId, qty:+qty, uom:material.uom, warehouseId:null, jobWorkerId:jwo.jobWorkerId, projectId:jwo.projectId, sourceType:'JobWorkOrder', sourceId:jwo.id, valuationRate:rate, actor});
